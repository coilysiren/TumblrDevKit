...
class Lexer(object):
    ...
    def scan(self, regexp, type):
        captures = Lexer.regexec(regexp, self.input)
    ...
    def consumeBlank(self):
        captures = Lexer.regexec(self.RE_BLANK, self.input)
        if not captures:
            return
    ...
    def tag(self):
        captures = Lexer.regexec(self.RE_TAG, self.input)
    ...
    def textBlockStart(self):
        captures = Lexer.regexec(self.RE_DOT_BLOCK_START, self.input)
        if captures is None:
            return
    ...
    def processInline(self, val):
        ...
        sval_replaced = Lexer.replace_string_brackets(sval)
        start_inline = self.RE_INLINE.search(sval_replaced).start()

        try:
            closing = start_inline + Lexer.detect_closing_bracket(sval_replaced[start_inline:])
        except IndexError:
    ...
    def prepend(self):
        captures = Lexer.regexec(self.RE_PREPEND, self.input)
        if captures:
            self.consume(len(captures[0]))
    ...
    def append(self):
        captures = Lexer.regexec(self.RE_APPEND, self.input)
        if captures:
            self.consume(len(captures[0]))
    ...
    def block(self):
        captures = Lexer.regexec(self.RE_BLOCK, self.input)
        if captures:
            self.consume(len(captures[0]))
    ...
    def assignment(self):
        captures = Lexer.regexec(self.RE_ASSIGNMENT, self.input)
        if captures:
            self.consume(len(captures[0]))
    ...
    def call(self):
        captures = Lexer.regexec(self.RE_CALL, self.input)
        if captures:
            self.consume(len(captures[0]))
    ...
    def conditional(self):
        captures = Lexer.regexec(self.RE_CONDITIONAL, self.input)
        if captures:
            self.consume(len(captures[0]))
    ...
    def each(self):
        captures = Lexer.regexec(self.RE_EACH, self.input)
        if captures:
            self.consume(len(captures[0]))
    ...
    def captureIndent(self):
        if self.indentRe:
            captures = Lexer.regexec(self.indentRe, self.input)
        else:
            regex = self.RE_INDENT_TABS
            captures = Lexer.regexec(regex, self.input)
